 ...
 *
 * Pendencias conhecidas (NAO implementadas nesta versao):
 *  - Filtro empresa MG/ML: depende do mapeamento empresa<->loja (ainda nao definido).
...
export class ListasService {
  ...
  // Telefone do cliente (DDD + numero) a partir do cpfcnpj da fashionstar.
  // adfashionstars.cpfcnpj vem SEM zeros a esquerda; erp_clientes_real guarda padded(14) -> lpad.
  private readonly telByFsCpf = `(
    SELECT NULLIF(btrim(coalesce(cr.clientes_ddd1,'')) || ' ' || btrim(coalesce(cr.clientes_telefone1,'')), '')
    FROM erp_clientes_real cr
    WHERE regexp_replace(cr.clientes_cpf_cnpj,'[^0-9]','','g') = lpad(regexp_replace(fs.cpfcnpj,'[^0-9]','','g'),14,'0')
    LIMIT 1)`;

  // Fragmento opcional: restringe aos clientes (codparc->cpf da fashionstar) da vendedora.
  ...
  async corrida(f: FiltroListaDto = {}) {
    ...
    const sql = `
      SELECT c.codparc,
             btrim(fs.nomeparc) AS nome,
             fs.cpfcnpj,
             ${this.telByFsCpf} AS telefone,
  ...
  async superOfensiva(f: SuperOfensivaFiltroDto = {}) {
    ...
    const fVend = this.filtroVendedoraPorFs(f.vendedora, params);
    const sql = `
      SELECT s.codparc, btrim(fs.nomeparc) AS nome, fs.cpfcnpj,
             ${this.telByFsCpf} AS telefone,
             s.meses_seguidos
      FROM (
  ...
  async desativacao(f: FiltroListaDto = {}) {
    ...
    const sql = `
      SELECT agg.doc AS cpfcnpj,
        (SELECT btrim(cr.clientes_nome) FROM erp_clientes_real cr
           WHERE regexp_replace(cr.clientes_cpf_cnpj,'[^0-9]','','g') = agg.doc LIMIT 1) AS nome,
        (SELECT NULLIF(btrim(coalesce(cr.clientes_ddd1,'')) || ' ' || btrim(coalesce(cr.clientes_telefone1,'')), '')
           FROM erp_clientes_real cr
           WHERE regexp_replace(cr.clientes_cpf_cnpj,'[^0-9]','','g') = agg.doc LIMIT 1) AS telefone,
